"""
YOLOv8 Model Wrapper - ultralytics-based implementation.
"""
...
class YOLOv8Model(BaseYOLOModel):
    ...
    SIZE_MAP = {"n": "yolov8n", "s": "yolov8s", "m": "yolov8m", "l": "yolov8l", "x": "yolov8x"}
    ...
    def predict(self, source, conf=0.25, iou=0.45, img_size=640, device="auto") -> list:
        results = self._ultra_model.predict(source=source, conf=conf, iou=iou, imgsz=img_size, device=device, verbose=False)
        return results
    ...
    def train(self, data, epochs=100, imgsz=640, batch=16, **kwargs):
        """Train using ultralytics built-in trainer."""
        results = self._ultra_model.train(data=data, epochs=epochs, imgsz=imgsz, batch=batch, **kwargs)
        return results
